...
export async function retryingFind(
    ...
    description: string, options: BrowserSideOptions, driver: WebDriver
  ...
  try {
    return await driver.wait(async () => {
      try {
        ...
        const response: WebElement|string|true =
            <any>(await driver.executeScript(
    ...
    // the test log.
    log.error(failure);
    log.error(`Browser-side find element tried ${returnTimes.length} times at:
